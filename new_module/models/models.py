                ...
                if largoQuery==0:
                    ...
                    for j in range(largoQuery):
                        if str(datosProveedor[0])==str(querySelect[j][0]):
                            ...
                            existe=False
                if existe==False:
                    cur.execute("insert into res_partner (id,vat, street, city,display_name ,l10n_cl_activity_description) values (1,'"+datosProveedor[0]+"','"+datosProveedor[1]+"','"+datosProveedor[2]+"','"+datosProveedor[3]+"','"+datosProveedor[4]+"');")
                    cur.execute(query)
                    querySelect = cur.fetchall()
                    largoQuery=len(querySelect)
                conn.commit()
